...
AIR_WHISPER_TEST_SOURCE="$(cd "$(dirname "$0")" && pwd)/bootstrap-llama.sh"
AIR_WHISPER_TEST_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/air-llama-bootstrap-test.XXXXXX")"
...
AIR_WHISPER_TEST_FIXTURE="$AIR_WHISPER_TEST_ROOT/fixture/build-apple/llama.xcframework"
AIR_WHISPER_TEST_MACOS="$AIR_WHISPER_TEST_FIXTURE/macos-arm64_x86_64/llama.framework"
...
printf 'original framework binary\n' > "$AIR_WHISPER_TEST_MACOS/llama"
printf 'fixture header\n' > "$AIR_WHISPER_TEST_MACOS/Headers/llama.h"
...
new_case() {
    ...
    sed "s/^AIR_WHISPER_CHECKSUM=.*/AIR_WHISPER_CHECKSUM=\"${2:-$AIR_WHISPER_TEST_CHECKSUM}\"/" \
        "$AIR_WHISPER_TEST_SOURCE" > "$AIR_WHISPER_TEST_CASE/scripts/bootstrap-llama.sh"
    AIR_WHISPER_TEST_FRAMEWORK="$AIR_WHISPER_TEST_CASE/Vendor/llama.xcframework"
    AIR_WHISPER_TEST_ARCHIVE="$AIR_WHISPER_TEST_CASE/Vendor/llama-b10896-xcframework.zip"
...
bootstrap() {
    bash "$AIR_WHISPER_TEST_CASE/scripts/bootstrap-llama.sh" > "$AIR_WHISPER_TEST_CASE/output.log" 2>&1
}
...
printf '%s\n' "$AIR_WHISPER_TEST_CHECKSUM" > "$AIR_WHISPER_TEST_CASE/Vendor/llama.sha256"
printf 'altered framework binary\n' > "$AIR_WHISPER_TEST_FRAMEWORK/macos-arm64_x86_64/llama.framework/llama"
...
AIR_WHISPER_TEST_INCOMPLETE="$AIR_WHISPER_TEST_ROOT/incomplete/build-apple/llama.xcframework"
