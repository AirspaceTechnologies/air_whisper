...
1. **`setup.sh`** — idempotent. Checks for Homebrew (exit with instructions if missing); installs hammerspoon, ffmpeg, whisper-cpp if absent; creates `~/.dictate/{models,tmp}`; downloads the model(s) with `curl -L` and minimum-size verification; defaults the fallback device to the system default input (zero questions; the 🎤 menu owns all later changes) and writes a fresh `~/.dictate/config.lua`; resolves and writes the whisper-server / whisper-cli paths; installs the Lua module into `~/.hammerspoon/dictate.lua` and appends `require("dictate")` to `~/.hammerspoon/init.lua` if not already present; launches Hammerspoon, whose **re-entrant onboarding wizard** handles the rest: fires the Accessibility and Microphone TCC prompts at the right moment with one line of why (auto-continuing after each grant, including the restart Accessibility needs), sets the Globe key to "Do Nothing" programmatically, and enables launch-at-login. The 🎤 menu shows live status rows (permissions / server / model). The mic fallback chain prefers built-in over Bluetooth mics so a dictation never silently drops headphone audio to call quality.
